...
class MyBB16Extractor
{
    ...
    {
        $query = "SELECT uid, username, email, postnum, FROM_UNIXTIME( regdate ) AS regdate, FROM_UNIXTIME( lastvisit ) AS lastvisit, usergroup, additionalgroups, avatar, lastip FROM {$this->getPrefix()}users";
        return $this->getGenerator($query, $offset);
    }
